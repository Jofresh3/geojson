...
# 'main_raw' 시트를 선택합니다.
worksheet = spreadsheet.worksheet('통계청_외식업')
...
numeric_columns = ['광고비사용비중','1인당급여','사업체수','종사자수','매출액(월)','임차료','광고선전비','영업이익','영업이익율','급여총액']
...
# '시/도' 선택을 위한 sidebar
# 전체도 할수 있도록 수정
selected_rgn1_nm = st.sidebar.selectbox('rgn1 선택', ['전체'] + list(df['시/도'].unique()))

# 선택된 '시/도'만 필터링
filtered_data = df if selected_rgn1_nm == '전체' else df[df['시/도'] == selected_rgn1_nm]
...
top5_data = filtered_data.groupby(['시/도', 'rgn2_nm'])[selected_info].sum().nlargest(5).reset_index()
...
* 광고비 사용 비중 = 광고 선전비를 전체 매출로 나눈 값으로 매출에서 광고 선전비가 차지하는 비중
* 1인당 급여 = 전체 인건비를 총 종업원수로 나눈 값으로 인건비의 높고 낮음을 확인할 수 있습니다.

단위: 명, %, 백만원            

raw 자료 통계청에서 참조하였습니다. [link](https://kosis.kr/)            
